...
class MMDataset(Dataset):
    ...
    def PLM_tokenizer (self, rawtexts):
        self.tokenizer = ChatGLMTokenizer.from_pretrained(self.args.pretrain_LM, trust_remote_code=True)
        token_list = []
...
def MMDataLoader(args):
    ...
    #DataLoader类的用法就是DataLoader()第一个参数为datasets，对DataLoader遍历时，
    # 会生成一组随机索引(若shuffle=True)，自动调用datasets的__getitem__方法(即datasets[...])，返回一个batch_size的batch
    return dataLoader